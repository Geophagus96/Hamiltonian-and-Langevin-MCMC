...
insideHAMS = function(delta, eps, n){
  s1s = vector(length = n)
  s2s = vector(length = n)
  u1s = vector(length = n)
  u2s = vector(length = n)
  ...
  for (i in 1:n){
    ...
    qup = prod(exp(oldgrad*(1-s)-(1/(2*delta^2))*(y-1)^2)+exp(oldgrad*(-1-s)-(1/(2*delta^2))*(y+1)^2))
    qdown = prod(exp(newgrad*(1-snew)-(1/(2*delta^2))*(y-1)^2)+exp(newgrad*(-1-snew)-(1/(2*delta^2))*(y+1)^2))
    log_ratio = f(snew)-f(s)+sum((oldgrad+newgrad)*(s-snew))
...
outsideresults = outsideHAMS(delta, eps, n)
